...
| mozna dzielic   |  |  |
| ``192.168.0.64/27``  |  | enp0s8 ``ip route add 192.168.0.64/27 via 192.167.100.98``|   ''ip rout addd {DESTINATION} via {ENDPOINT}''
| ``192.168.0.128/27`` |  | enp0s8 ``ip route add 192.168.0.128/27 via 192.167.100.98``|
| ``192.168.0.192/27`` |  | enp0s8 ``ip route add 192.168.0.192/27 via 192.167.100.98``|
| lub za 1 zamachem   |  |  |
| ``192.168.0.0/24``   |  | enp0s8 ``ip route add 192.168.0.0/16 via 192.167.100.98``| #jednym wpisem można wykonać powyższe operacje
| ``PC-R-2``  |  |        |
| ``192.168.200.0/24`` |  | enp0s3 |
| ``192.168.100.0/24`` |  | enp0s3 ``ip route add 192.168.0.0/24 via 192.167.100.97``|
